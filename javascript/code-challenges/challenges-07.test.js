...
const howMuchPencil = (str) => {
  ...
  for (let i = 0; i <= str.length; i++) {
    result.push(str.split('').slice(i, str.length).join(''));
  }
  return result;
};
...
const listFoods = (recipe) => {
  ...
  // Solution code here...
  let arr = [];
  recipe.ingredients.map(item => arr.push(item.slice(item.indexOf(' ') + 1, item.length)));
  arr.map(item => result.push(item.slice(item.indexOf(' ') + 1, item.length)));
  return result;
};
