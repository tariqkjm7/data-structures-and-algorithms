...
const count = (target, input) => {
  // Solution code here...
  let Times = input.filter(item => item.includes(target) ? true : false).reduce((accEnd, item) => accEnd = item.reduce((acc, number) => target === number ? accEnd += 1 : accEnd, 0), 0);
  return Times;
};
...
const totalSum = (input) => {
  // Solution code here...
  let Sum = input.reduce((accEnd, item) => accEnd = item.reduce((acc, number) => accEnd += number, 0), 0)
  return Sum;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let names = [];
  Object.values(data).forEach(item => item.gender === 'male' || item.gender === 'female' ? names.push(item.name) : false);
...
let findShortest = (data) => {
  // Solution code here...
  let Test = data.reduce((acc, item) => acc < item.height ? acc = item.height : acc, 0);
  let short = data.reduce((acc, item) => item.height === Test ? acc = item.name : acc, '');
  return short;
};
